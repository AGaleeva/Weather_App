...
const WEATHER_TEMPERATURE = document.querySelector("#weather-temp");

const WEATHER_ERROR = document.querySelector("#weather-info-container");
const WEATHER_ERROR_INFO = document.querySelector(
  "#weather-info-container #weather-city"
);

// Наш APP_ID
const APP_ID = "e6393aad6b894d00cd609e72c2eaa034";
...
const createErrorWeaterCard = (weatherData) => {
  LOADING_TEXT.style.display = "none";
  WEATHER_INFO_CONTAINER.style.display = "flex";

  WEATHER_ICON.style.display = "none";

  WEATHER_ERROR.style.flexDirection = "column";
  WEATHER_ERROR.style.justifyContent = "center";
  WEATHER_CITY.textContent = "API Error";
  WEATHER_ERROR_INFO.style.color = "#F35E5E";      
  WEATHER_ERROR_INFO.style.fontSize = "57px";      
  WEATHER_TEMPERATURE.textContent = `${weatherData.cod}: ${weatherData.message}`;  
};
...
async function searchWeatherForCity() {
  ...
  try {
    ...
    // тут нужно будет написать код по работае сошибками и их отображениямт на экране
    createErrorWeaterCard(error.response);
  }
}
